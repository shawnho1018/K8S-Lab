...
# This tag enables the global mtls among services. It is visible using kiali. You could also run get-spiffe.sh to check istio self-signed certificates. 
...
# This tag enable token to access productpage. To get the login token, please call gen-token-query.sh and uses it in your curl request as curl --header "Authorization: Bearer $TOKEN" $IP/productpage -s -o /dev/null -w "%{http_code}\n"
